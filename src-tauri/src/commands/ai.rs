...
#[tauri::command]
pub async fn get_openrouter_free_models(ai_router: State<'_, AiRouter>) -> Result<Vec<String>, String> {
    ai_router.fetch_openrouter_free_models().await.map_err(|e| e.to_string())
}

#[tauri::command]
pub fn test_intent_router(intents: Vec<String>, ai_router: State<'_, AiRouter>) -> Result<Vec<(String, String)>, String> {
    let scores = ai_router.score_sample_intents(intents);
    Ok(scores.into_iter().map(|(intent, tier)| (intent, format!("{:?}", tier))).collect())
}

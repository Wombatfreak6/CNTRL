...
fn test_browser_service_lifecycle() {
    ...
    // 1. Open tab
    let id = service.open_tab("https://example.com".to_string(), false).unwrap();
    ...
    // 2. Navigate
    service.navigate(id, "https://news.ycombinator.com".to_string()).unwrap();
    let tabs = service.get_tabs().unwrap();
    assert_eq!(tabs[0].url, "https://news.ycombinator.com");

    // 3. Open background tab
    let bg_id = service.open_tab("https://reddit.com".to_string(), true).unwrap();
    let tabs = service.get_tabs().unwrap();
